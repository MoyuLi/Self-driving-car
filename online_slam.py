...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  6 21:59:57 2018
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        list2 = range(2,len(Omega.value))
        A = Omega.take([0,1],list2)
        B = Omega.take([0,1],[0,1])
        C = Xi.take([0,1],[0])        
        
        Omega = Omega.take(list2,list2) - A.transpose()*B.inverse()*A
        Xi = Xi.take(list2,[0]) - A.transpose()*B.inverse()*C
